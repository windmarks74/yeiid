...
//   - 로케일 = 기기 언어 자동감지. 한국어면 'ko', 그 외는 전부 'en'.
//   - 한국어 기기는 그대로 ko 라 기존 사용자 영향 없음. (2026-09-20 기준이 뒤집혔다 —
//     예전엔 "영어면 en, 그 외 ko" 라서 일본·독일 기기에 한글이 떴다. detectLang 주석 참고.)
...
function detectLang(): Lang {
  try {
    ...
    // 한국어면 한국어, 나머지는 전부 영어.
    //
    // 예전에는 반대였다("영어면 영어, 그 외는 한국어") → **일본·독일·프랑스 기기에서
    // 한글이 떴다.** 한국 밖에 출시하면 그 사용자들은 읽을 수 없는 화면을 보게 된다.
    // 영어가 정답은 아니어도 한국어보다는 낫다.
    //
    // ⚠️ 맞바꾼 것: 기기 언어를 한국어도 영어도 아닌 것으로 쓰는 **한국 사용자**는
    //    이제 영어 화면을 보고, 시험 규격(큐넷·공무원·토익·KPC)이 숨겨진다
    //    (isSelectable 이 LANG === 'ko' 로 막기 때문). 드문 경우라 감수한다.
    //    출시 국가가 늘면 시험 규격은 **언어가 아니라 지역**으로 가르는 게 맞다.
    return l.startsWith('ko') ? 'ko' : 'en'
  } catch {
    // 로케일을 못 읽는 경우. 현재 사용자가 대부분 한국이라 한국어로 둔다.
    return 'ko'
  }
}
...
const STRINGS = {
  ...
  'usage.kpc.notice.b1': { ko: '단색 배경(별도 배경 없음)·컬러, 정면 상반신(어깨까지), 모자 없이 정수리~턱 노출, 최근 6개월 이내.', en: '단색 배경(별도 배경 없음)·컬러, 정면 상반신(어깨까지), 모자 없이 정수리~턱 노출, 최근 6개월 이내.' },
  'usage.kpc.notice.b2': { ko: '안경은 반사·눈가림 없이(권고). 과도한 보정은 피하세요.', en: '안경은 반사·눈가림 없이(권고). 과도한 보정은 피하세요.' },
  'usage.kpc.notice.b3': { ko: '디지털 업로드는 JPG/PNG · 가로 115~235px·세로 150~315px · 500KB 이하 — 225×300px로 자동 맞춤.', en: '디지털 업로드는 JPG/PNG · 가로 115~235px·세로 150~315px · 500KB 이하 — 225×300px로 자동 맞춤.' },
  ...
  'usage.gosi.notice.title': { ko: '공무원 채용(국가공무원채용시스템) 사진 안내', en: '공무원 채용(국가공무원채용시스템) 사진 안내' },
  'usage.gosi.notice.b1': { ko: '단색 배경·정면 상반신, 이마와 귀가 보이게, 최근 6개월 이내 촬영(권장).', en: '단색 배경·정면 상반신, 이마와 귀가 보이게, 최근 6개월 이내 촬영(권장).' },
  'usage.gosi.notice.b2': { ko: '안경은 반사·눈가림 없이(권고). 본인 식별이 명확해야 합니다.', en: '안경은 반사·눈가림 없이(권고). 본인 식별이 명확해야 합니다.' },
  'usage.gosi.notice.b3': { ko: '디지털 업로드는 JPG/PNG · 137×177px · 350KB 미만 — 자동 맞춤.', en: '디지털 업로드는 JPG/PNG · 137×177px · 350KB 미만 — 자동 맞춤.' },
  ...
  'usage.toeic.notice.b1': { ko: '흰색 배경, 정면, 두 귀가 보이게 어깨까지, 정수리~턱 3.2~3.6cm, 최근 6개월 이내.', en: '흰색 배경, 정면, 두 귀가 보이게 어깨까지, 정수리~턱 3.2~3.6cm, 최근 6개월 이내.' },
  'usage.toeic.notice.b2': { ko: '안경은 반사·눈가림 없이(권고). 과도한 보정은 피하세요.', en: '안경은 반사·눈가림 없이(권고). 과도한 보정은 피하세요.' },
  'usage.toeic.notice.b3': { ko: '디지털 업로드는 JPG · 6MB 이하(앱은 훨씬 작게 저장) · 300×400px로 자동 맞춤.', en: '디지털 업로드는 JPG · 6MB 이하(앱은 훨씬 작게 저장) · 300×400px로 자동 맞춤.' },
  ...
  'app.billingCaption': { ko: '무료 {n}장 제공 · 추가는 {price} 평생', en: '{n} free downloads · more for {price}, lifetime' },
  ...
  'app.landingCap': { ko: '무료로 {n}장 다운로드 · 가입 없이 바로 시작', en: '{n} free downloads · start right away, no sign-up' },
